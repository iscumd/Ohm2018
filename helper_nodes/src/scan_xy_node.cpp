...
struct scan_transformer {
  ...
  void scan_to_XY_tf(const sensor_msgs::LaserScan::ConstPtr& scan_in) {
    ...
    if(!listener.waitForTransform(scan_in->header.frame_id, "base", scan_in->header.stamp + ros::Duration().fromSec(scan_in->ranges.size()*scan_in->time_increment), ros::Duration(1.0))) {
      ROS_INFO("scan_to_XY_tf: waitForTransform failed");
      return;
    }
  
    sensor_msgs::PointCloud cloud;
    projector_.transformLaserScanToPointCloud("/base", *scan_in, cloud, listener);
